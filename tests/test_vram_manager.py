...
class TestVRAMManager:
    ...
    @pytest.fixture
    def vram(self):
        # Pin the budget to the no-GPU fallback (5.5 GB) so these tests are
        # deterministic regardless of the host's actual detected VRAM.
        mgr = VRAMManager()
        mgr.VRAM_LIMIT = 5.5
        mgr._parallel_capable = mgr.VRAM_LIMIT >= 8.0
        return mgr
